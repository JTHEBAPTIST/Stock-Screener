...
def run_optimizer(sector_selection, risk_aversion, tracking_error_limit, optimization_type='Mean-Variance'):
    ...
    # Download historical prices from yfinance
    price_data = yf.download(tickers, start="2021-01-01", end="2024-01-01")["Adj Close"]
    price_data = price_data.dropna(axis=1)  # Drop tickers with missing data

    # Calculate daily returns
    returns = price_data.pct_change().dropna()

    # Re-map tickers to sectors (some may have dropped)
